...
/**
 * Page that displays and manages pipeline templates.
 *
 * Renders a header with a "New Template" button that opens a create dialog, shows a loading
 * indicator while templates load, displays an empty state when none exist, and renders a
 * responsive grid of template cards for viewing, editing, and deleting templates.
 *
 * @returns A React element rendering the pipeline templates management UI.
 */
...
/**
 * Renders a card showing a pipeline template's name, stages, and actions to manage it.
 *
 * Shows the template title and count of stages, a badge list of stages, and controls to manage team assignments, edit the template, and delete it.
 *
 * @param template - Template data including `_id`, `name`, `stages`, and optional `assignedUserIds`, `followerIds`, and `assignerIds`.
 * @returns A React element representing the template card with action controls for team management, editing, and deletion.
 */
...
/**
 * Form for creating or editing a pipeline template.
 *
 * Renders inputs for the pipeline name and an ordered list of stages (with add, remove, edit, and drag-and-drop reordering), and handles create or update mutations.
 *
 * @param initialData - Optional existing template to edit; should include `_id`, `name`, and `stages` (array of stage names). When provided, the form initializes in edit mode.
 * @param onSuccess - Callback invoked after a successful create or update (or when the Cancel button is pressed) to allow the parent to close the form or refresh data.
 * @returns The form element that manages pipeline template creation and editing.
 */
...
/**
 * A draggable, editable row representing a pipeline stage.
 *
 * @param stage - The stage object containing `id` and `name`.
 * @param index - Zero-based position of the stage in the list.
 * @param onChange - Called with `(id, newName)` when the stage name is edited.
 * @param onRemove - Called with `(id)` when the stage's remove action is triggered.
 * @returns A JSX element rendering a sortable stage item with a drag handle, index badge, editable name input, and remove button.
 */
