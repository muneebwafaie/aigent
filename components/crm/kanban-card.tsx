...
/**
 * Render a draggable Kanban card representing an opportunity with optional overlay, pending state, and context-menu actions.
 *
 * @param opportunity - Opportunity data used to populate title, probability, value, creation time, expected close date, and current stage
 * @param stages - Optional list of stage names used to build the "Move to Stage" submenu; current stage is disabled
 * @param isOverlay - When true, applies a visual overlay style (ring, rotation, shadow)
 * @param isPending - When true, shows a syncing overlay, reduces interactivity and visual prominence
 * @param onClick - Optional click handler for the card (ignored while pending)
 * @param onMove - Optional callback invoked with the target stage index when a stage is selected from the move submenu
 * @param onDelete - Optional callback invoked when the Delete action is selected
 * @param onEdit - Optional callback invoked when the Edit action is selected
 * @returns The rendered Kanban card element configured for drag-and-drop and context-menu interactions
 */
