...
/**
 * Determine whether the given identity has read access to the specified opportunity under the "org:opportunities" permission model.
 *
 * @param identity - The authenticated identity to check permissions for
 * @param opp - The opportunity document to evaluate access against
 * @returns `true` if the identity can read `opp`, `false` otherwise
 */
...
/**
 * Determine whether an identity may update the given opportunity under the org:opportunities permission model.
 *
 * @returns `true` if the identity has permission to update `opp`, `false` otherwise.
 */
